<template>
	<div class="ly-tree-node" @click.stop="handleClick" v-show="node.visible" :class="{ 'is-expanded': expanded, 
			'is-hidden': !node.visible, 
			'is-checked': !node.disabled && node.checked }"
	 role="treeitem" name="LyTreeNode" ref="node">
		<div class="ly-tree-node__content" :class="{'is-current': node.isCurrent && highlightCurrent}" :style="{ 'padding-left': (node.level - 1) * indent + 'px' }">
			<span @click.stop="handleExpandIconClick" :class="[{ 'is-leaf': node.isLeaf, expanded: !node.isLeaf && node.expanded }, 'ly-tree-node__expand-icon', iconClass ? iconClass : 'ly-iconfont ly-icon-caret-right']"></span>
			...
			<ly-checkbox v-if="checkboxVisible || radioVisible"
				...
				@check="handleCheckChange(!node.ccheked)"/>
			
			<span v-if="node.loading" class="ly-tree-node__loading-icon ly-iconfont ly-icon-loading"></span>
			...
			<template v-if="node.icon && node.icon.length > 0">
				...
				<span v-else class="ly-tree-node__icon" :class="node.icon"></span>
			</template>
			
			<span class="ly-tree-node__label">{{node.label}}</span>
		</div>
		
		<div v-if="!renderAfterExpand || childNodeRendered" v-show="expanded" class="ly-tree-node__children" role="group">
